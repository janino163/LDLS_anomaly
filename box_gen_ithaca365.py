...
from ithaca365.utils.geometry_utils import BoxVisibility, transform_matrix
...
def get_lidar_boxes_ithaca365(pc, mask, name, fit_method='min_zx_area_fit', pointsensor_token=None, nusc=None, vis=False):
...
#             plot_pcd_one_view(os.path.join("", '{:03d}.png'.format(i)), [input_ptc[0].detach().cpu().numpy(), fine[0].detach().cpu().numpy()], ['Input', 'Output'], xlim=(-0.35, 0.35), ylim=(-0.35, 0.35), zlim=(-0.35, 0.35))
        ...
        bbox = box_kiiti_to_ithaca365(obj, pointsensor_token, name, nusc)
...
def test_dbscan(pc, mask, name, fit_method='min_zx_area_fit', pointsensor_token=None, nusc=None, vis=False):
    import itertools
...
#         sample = nusc.get("sample_data", sample_data_token)
            
        # Transforms
        pointsensor_token = sample_data_token[0]
        cam_sample_data_tokens = sample_data_token[1:]
        
#         pointsensor_token = sample['LIDAR_TOP']
        ...
        ldls_mask_path = osp.join(args.data_paths.ldls_full_mask_path, f"{pointsensor_token}.npz")
        with np.load(ldls_mask_path) as data:
            car_mask = data['car']
            ped_mask = data['pedestrian']
            
        car_boxes = get_lidar_boxes_ithaca365(pc_ori, car_mask, name='car', fit_method='PCA', pointsensor_token=pointsensor_token, nusc=nusc, vis=visualize)
        pedestrian_boxes = get_lidar_boxes_ithaca365(pc_ori, car_mask, name='pedestrian', fit_method='PCA', pointsensor_token=pointsensor_token, nusc=nusc, vis=visualize)
        
        np.savez(osp.join(args.data_paths.ldls_full_box_path, f"{pointsensor_token}"), pedestrian=pedestrian_boxes, car=car_boxes)
