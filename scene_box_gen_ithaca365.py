...
def save_masks(args, name, mask_ped, mask_car, obj_mask, p2):
    if not os.path.exists(args.data_paths.ithaca365_masks):
        os.makedirs(args.data_paths.ithaca365_masks)
...
def plot_pedestrian_lidar(lidar):
    ...
    fig.write_html(f"/home/jan268/temp/predictions_lidar.html")
...
def main(args: DictConfig):
    ...
    detector = MaskRCNNDetector()
    sam_path =  osp.join(args.data_paths.ldls_sample_path, "all_samples.csv")
#     sam_path =  f"/home/jan268/repo/LDLS_anomaly/missing.csv"
    
    data = pd.read_csv(sam_path, header=None)
    
    sample_data_tokens = data.values.tolist()
#     sample_data_tokens = sample_data_tokens[args.start_index:args.end_index]
#     print(len(sample_data_tokens))
#     exit()
#     sample_data_tokens = sample_data_tokens[0:27500]
...
#         sample_data_tokens.extend(tracked_data.values.tolist())
    ...
    for sample_data_token in tqdm(sample_data_tokens):
        ...
        if os.path.exists(osp.join(args.data_paths.ithaca365_masks, f"{base_name}.npz")):
            ...
            missing_samples.append(sample_data_token)
            
    with open(f"/home/jan268/repo/LDLS_anomaly/missing.csv", "w") as f:
        writer = csv.writer(f)
        writer.writerows(missing_samples)
        
    sample_data_tokens = missing_samples
    print(len(sample_data_tokens))
    for sample_data_token in tqdm(sample_data_tokens):
        lidar_sample_data_token = sample_data_token[0]
        pointsensor = nusc.get('sample_data', lidar_sample_data_token)
    ...
    #         base_name = lidar_sample_data_token
        ...
        if os.path.exists(osp.join(args.data_paths.ithaca365_masks, f"{base_name}.npz")):
            ...
            H = get_point_persistency_score(lidar_sample_data_token, nusc, num_histories=5, ranges=(-10, 70))
...
#             detection.visualize_ithaca365(camera_channel, nusc)
        ...
        save_masks(args, filename, mask_ped=ped_seg_mask, mask_car=car_seg_mask, obj_mask=object_mask, p2=H)
        # save box object
        if not os.path.exists(args.data_paths.ithaca365_boxes):
            os.makedirs(args.data_paths.ithaca365_boxes)
        np.savez(osp.join(args.data_paths.ithaca365_boxes, f"{base_name}"), pedestrian=ped_boxes, car=car_boxes)
