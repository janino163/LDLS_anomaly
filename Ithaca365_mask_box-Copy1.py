...
def project_to_image(lidar_data_token, nusc, camera_data_token):
    ...
    lidar = copy.deepcopy(pc.points[:3, :])
#     plot_pedestrian_lidar(lidar)
#     exit()
...
def mask_points(detection, lidar_cam, lidar, depths, im, object_mask, tag=''):
    ...
    fov_mask = np.logical_and(fov_mask, lidar_cam[0, :] > 0)
    fov_mask = np.logical_and(fov_mask, lidar_cam[0, :] < im.size[0])
    fov_mask = np.logical_and(fov_mask, lidar_cam[1, :] > 0)
    fov_mask = np.logical_and(fov_mask, lidar_cam[1, :] < im.size[1])
#     plot_pedestrian_lidar(lidar[:3, fov_mask], tag=f'fov_{tag}')
...
#         print(max_lidar[0,:].mean(), max_lidar[1,:].mean(), max_lidar[2,:].mean())
...
def box_to_lidar(nusc, lidar_sd_token, box):
    lidar_sd = nusc.get('sample_data', lidar_sd_token)
    # Third step: transform from global into the ego vehicle frame for the timestamp of the image.
    poserecord = nusc.get('ego_pose', lidar_sd['ego_pose_token'])
    box.translate(-np.array(poserecord['translation']))
    box.rotate(Quaternion(poserecord['rotation']).inverse)
    
    # Fourth step: transform from ego into the lidar.
...
#         detection.visualize_nusc(camera_channel, nusc)
...
def plot_pedestrian_lidar(lidar, tag=''):
    ...
    fig.write_html(f"/home/jan268/temp/lidar_{tag}.html")
...
#     sample_data_tokens = sample_data_tokens[27500:55000]
#     sample_data_tokens = sample_data_tokens[114360:]


    eval_scenes = ['00e798b0fc0f984f2d3b65b1fd5c2bd3',
    'd8cc0305881ddea482b9af3881f2e214',
    '00b22e0f57e6d52240f4dd43bd9cabd9',
    'c226366bd789ffbd1e38f8f276be9413']
    sample_data_tokens = []
...
#     sample_data_tokens = [['c6d3fff945513004738aedd9fc902cfb', '9010a6d6f6bacda48df3b5596d1c138e', '9502b5af59f9024fbae22041fc04d297']]
#     sample_data_tokens = [['6e048e946d2595dd30c43386198f995a', 'a36fcee75e23a4fd9d07d7115a69159a', 'd2f91c703c76d5f8db7b1dd06cf1d5f8']]
#     sample_data_tokens = [['de16564b08dc65554682eeca7c3ffa39', '108bc6e24ded84c989baf634ee12bb22', '14793e001741eb0cbd6fdf4540de6aca']]
...
#             detection.visualize_ithaca365(camera_channel, nusc)
            ...
            temp_car_seg_mask, temp_ped_seg_mask, ped_cluster, car_cluster = mask_points(detection, lidar_cam, lidar, depths, im, object_mask, tag=camera_channel)
            car_objs = get_objs(car_cluster, 'car')
            for car in car_objs:
                car_boxes.append(box_kiiti_to_ithaca365(car, lidar_sample_data_token, 'car', nusc))
...
#             plot_pedestrian_lidar(lidar[:3, temp_ped_seg_mask], tag=f'{camera_channel}')
            
#         plot_pedestrian_lidar(lidar[:3, ped_seg_mask], tag=f'whole')
        filename = f'{base_name}'
        # save Mask-rccn + object mask, pure object mask, p2 score
        save_masks(args, filename, mask_ped=ped_seg_mask, mask_car=car_seg_mask, obj_mask=object_mask, p2=H)
        # save box object
...
#         nusc.render_sample_data(lidar_sample_data_token, underlay_map=False, with_anns=False, ax=ax, use_flat_vehicle_coordinates=False)
#         for box in ped_boxes:
#             c = np.array(nusc.colormap[box.name]) / 255.0
# #             box = box_to_lidar(nusc, lidar_sample_data_token, box)
#             box.render(ax, view=np.eye(4), colors=(c, c, c))
#         plt.savefig('/home/jan268/temp/box_temp_fig.png')
